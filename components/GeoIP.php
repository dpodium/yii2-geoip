...
// Copyright 2009 Maxmind LLC All Rights Reserved
    ...
    protected function setupSegments()
    {
        ...
        if ($this->flags & self::SHARED_MEMORY) {
            ...
            for ($i = 0; $i < self::STRUCTURE_INFO_MAX_SIZE; $i++) {
                ...
                if ($delim == (chr(255) . chr(255) . chr(255))) {
                    ...
                    if ($this->databaseType === self::REGION_EDITION_REV0) {
                        ...
                        || ($this->databaseType === self::ORG_EDITION_V6)
                        || ($this->databaseType === self::DOMAIN_EDITION)
                        || ($this->databaseType === self::DOMAIN_EDITION_V6)
                        || ($this->databaseType === self::ISP_EDITION)
                        || ($this->databaseType === self::ISP_EDITION_V6)
                        || ($this->databaseType === self::USERTYPE_EDITION)
                        || ($this->databaseType === self::USERTYPE_EDITION_V6)
                        || ($this->databaseType === self::LOCATIONA_EDITION)
                        || ($this->databaseType === self::ACCURACYRADIUS_EDITION)
                        || ($this->databaseType === self::CITY_EDITION_REV0_V6)
                        || ($this->databaseType === self::CITY_EDITION_REV1_V6)
                        || ($this->databaseType === self::NETSPEED_EDITION_REV1)
                        || ($this->databaseType === self::NETSPEED_EDITION_REV1_V6)
                        || ($this->databaseType === self::ASNUM_EDITION)
                        || ($this->databaseType === self::ASNUM_EDITION_V6)
                    ) {
            ...
            if (($this->databaseType == self::COUNTRY_EDITION) ||
                ...
                ($this->databaseType == self::PROXY_EDITION) ||
                ($this->databaseType == self::NETSPEED_EDITION)
            ) {
    ...
    protected function lookupCountryId($addr)
    {
        if (filter_var($addr, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
            ...
            if ($this->databaseType !== self::COUNTRY_EDITION_V6) {
                throw new Exception("Invalid database type; lookupCountry*() methods expect Country database.");
            }
            if ($ipnum === false) {
                throw new Exception("Invalid IP address: " . var_export($addr, true), self::ERR_INVALID_IP);
            }
            return $this->seekCountryv6($ipnum) - self::COUNTRY_BEGIN;
        }
    }
    ...
    protected function seekCountryv6($ipnum)
    {
    ...
    public function lookupLocation($addr)
    {
        if (filter_var($addr, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
            ...
            $ipnum = inet_pton($addr);
            if ($this->databaseType !== self::CITY_EDITION_REV0_V6 && $this->databaseType !== self::CITY_EDITION_REV1_V6) {
                throw new Exception("Invalid database type; lookupLocation() methods expect City database.");
            }
            if ($ipnum === false) {
                throw new Exception("Invalid IP address: " . var_export($addr, true), self::ERR_INVALID_IP);
            }
            return $this->getRecordv6($ipnum);
        }
    }
    ...
    protected function getRecordv6($ipnum) {
        $seek_country = $this->seekCountryv6($ipnum);
        if ($seek_country == $this->databaseSegments) {
            return null;
        }
        return $this->_common_get_record($seek_country);
    ...
    protected function _common_get_record($seek_country)
    {
        ...
        if ($this->flags & self::MEMORY_CACHE) {
            $record_buf = substr($this->memoryBuffer, $record_pointer, self::FULL_RECORD_LENGTH);
        } elseif ($this->flags & self::SHARED_MEMORY) {
